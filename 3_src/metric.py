"""Reference metric — verbatim port of the host notebook
https://www.kaggle.com/code/florianderoofr/slash-poker-competition-metric
Score = 0.70*PairAP + 0.20*EvidenceMAP@5 + 0.10*BehaviorMAP (macro over 3 disclosed families)
"""
import numpy as np
import pandas as pd
...
def _clean_evidence(values: list) -> list:
    cleaned = []
    for value in values:
        if pd.isna(value):
            continue
...
def score(
    ...
    `return_components=True` (local extension) also returns the three parts.
    """
    ...
    if return_components:
        return float(final_score), {
            ...
            "behavior_ap_per_family": dict(zip(TARGET_BEHAVIORS, behavior_scores)),
        }
    return float(final_score)
